...
4. Verify the install with the final-check chunk in [Module 0](articles/00_setup.html)

> **Optional: backup checkpoints.** A separate Zenodo record hosts
> pre-computed checkpoints (`01_qc_filtered.rds`, `02_integrated_clustered.rds`,
> `03_annotated.rds`, `afternoonSession.zip`) so you can start at any
> module boundary. `afternoonSession.zip` contains the Session 2
> (Module 5, 6, 7) input and intermediate results — unzip and add the
> files to `data/` and `results/` per the instructions. See the optional
> "Backup Checkpoints" subsection in [Module 0](articles/00_setup.html).
